...
/// Utility `mixin` for validating `String` emails
mixin EmailValidator {
  ...
  static bool _skipAtom(String text, bool allowInternational) {
    ...
    while (_index < text.length && _isAtom(text[_index], allowInternational)) _index++;
  ...
  static bool _skipSubDomain(String text, bool allowInternational) {
    ...
    while (_index < text.length && _isDomain(text[_index], allowInternational)) _index++;
  ...
  static bool _skipQuoted(String text, bool allowInternational) {
    ...
    while (_index < text.length) {
      if (text[_index].codeUnitAt(0) >= 128 && !allowInternational) return false;
  ...
  static bool _skipIPv4Literal(String text) {
    ...
    while (_index < text.length && groups < 4) {
      ...
      if (_index == startIndex || _index - startIndex > 3 || value > 255) return false;
  ...
  static bool validate(String? email,
    ...
    if (email[_index] == '"') {
      if (!_skipQuoted(email, allowInternational) || _index >= email.length) return false;
    } else {
      if (!_skipAtom(email, allowInternational) || _index >= email.length) return false;
    ...
    if (_index + 1 >= email.length || _index > 64 || email[_index++] != '@') return false;

    if (email[_index] != '[') {
      // domain
      if (!_skipDomain(email, allowTopLevelDomains, allowInternational)) return false;
